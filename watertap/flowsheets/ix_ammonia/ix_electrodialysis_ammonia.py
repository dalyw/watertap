...
from idaes.models.unit_models import Product, Feed, Separator  # ADDED SEPARATOR
from idaes.core.util import DiagnosticsToolbox

from watertap.core.util.initialization import check_dof
from watertap.property_models.multicomp_aq_sol_prop_pack import MCASParameterBlock
from watertap.unit_models.ion_exchange_0D import IonExchange0D
from watertap.unit_models.electrodialysis_1D import Electrodialysis1D  # ADDED
...
def ix_build(ions, target_ion=None, hazardous_waste=False, regenerant="NaCl"):
    ...
    # DIFFERENT THAN IX - Add Electrodialysis1D stack
    ...
    # DIFFERENT THAN IX - Add separator before inlet to Electrodialysis1D
    m.fs.separator = Separator(
        property_package=m.fs.properties,
        outlet_list=["outlet_to_diluate", "outlet_to_concentrate"],
    )
    ...
    # DIFFERENT THAN IX - add costing block for the electrodialysis stack
    ...
    # DIFFERENT THAN IX - Connect regen to the separator, separator to Electrodialysis1D, (Electrodialysis1D back to regen?)
    m.fs.ix_regen_to_separator = Arc(source=ix.regen, destination=m.fs.separator.inlet)
    m.fs.separator_to_ed_dil = Arc(
        source=m.fs.separator.outlet_to_diluate, destination=m.fs.EDstack.inlet_diluate
    )
    m.fs.separator_to_ed_conc = Arc(
        source=m.fs.separator.outlet_to_concentrate,
...
def set_operating_conditions(m, flow_in=0.05, conc_mass_in=0.0001, solver=None):
    ...
    m.fs.feed.properties.calculate_state(
        var_args={
            ...
            ("conc_mass_phase_comp", ("Liq", "Na_+")): 0,  # Not in feed
            ("conc_mass_phase_comp", ("Liq", "Cl_-")): 0,  # Not in feed
    ...
    # DIFFERENT THAN IX - copied "Fix ED unit vars" from electridialysis.1stack.py

    # Fix separator's split_fraction to 0.5, i.e., equal flows into the diluate and concentrate channels
    m.fs.separator.split_fraction[0, "outlet_to_diluate"].fix(0.5)
    ...
    m.fs.EDstack.voltage_applied.fix(0.5)
    ...
    # Set diffusivity and transport numbers for all ions in ED stack
    ...
    m.fs.EDstack.solute_diffusivity_membrane["cem", "NH4_+"].fix(1.8e-10)
    m.fs.EDstack.solute_diffusivity_membrane["aem", "NH4_+"].fix(1.25e-10)
    ...
    m.fs.EDstack.ion_trans_number_membrane["cem", "NH4_+"].fix(1)
    m.fs.EDstack.ion_trans_number_membrane["aem", "NH4_+"].fix(0)
...
def initialize_system(m):
    ...
    # DIFFERENT THAN IX - propagate to separator then ED stack
    propagate_state(m.fs.ix_regen_to_separator)
    m.fs.separator.initialize()

    # ADDED FROM CLAUDE 3.5: Improve EDstack initialization feasibility
    # Get values from regeneration stream - same concentrations as after separator
    for ion in ["Na_+", "Cl_-", "NH4_+"]:
        print(
            f'Regen stream conc for {ion}: {value(m.fs.ion_exchange.regeneration_stream[0].conc_mass_phase_comp["Liq", ion])}'
        )

    # Set the value for Na_+ and Cl_- to 8 in the regeneration stream
    m.fs.ion_exchange.regeneration_stream[0].conc_mass_phase_comp[
        "Liq", "Na_+"
    ].set_value(8)
    m.fs.ion_exchange.regeneration_stream[0].conc_mass_phase_comp[
        "Liq", "Cl_-"
    ...
    print(
        f"Set Na_+ concentration in regeneration stream to: {value(m.fs.ion_exchange.regeneration_stream[0].conc_mass_phase_comp['Liq', 'Na_+'])}"
    )
    print(
        f"Set Cl_- concentration in regeneration stream to: {value(m.fs.ion_exchange.regeneration_stream[0].conc_mass_phase_comp['Liq', 'Cl_-'])}"
    )

    propagate_state(m.fs.separator_to_ed_dil)
    propagate_state(m.fs.separator_to_ed_conc)

    # Print from separator outlets (which feed to ED stack)
    # Print all attributes of the ScalarPort to see what's available
    # Based on the attributes list, we can try accessing concentrations through using 'vars' to get variables
    for var in list(m.fs.separator.outlet_to_diluate.iter_vars()):
        print(f"  - {var.name}")

    for var in list(m.fs.separator.outlet_to_concentrate.iter_vars()):
        print(f"  - {var.name}")

    print(
        f"Degrees of freedom on EDstack before initialization: {degrees_of_freedom(m.fs.EDstack)}"
    )
...
#     # DIFFERENT THAN ORIGINAL IX: For set outlet concentration equal to inlet concentration for Na+ and Cl-
#     # for ion in m.fs.properties.ion_set:
#     #     if ion != target_ion:
#     #         inlet_conc = ix.process_flow.properties_in[0].conc_mass_phase_comp["Liq", ion]()
#     #         ix.process_flow.properties_out[0].conc_mass_phase_comp["Liq", ion].fix(inlet_conc)
...
#     # DIFFERENT THAN IX - reinitialize separator and ED
#     propagate_state(m.fs.ix_regen_to_separator)
#     m.fs.separator.initialize()
...
#     # ADDED FROM CLAUDE 3.5: Get values from separator outlets for better initial guesses
#     dil_conc = {}
#     conc_conc = {}
#     for ion in m.fs.properties.ion_set:
#         dil_conc[ion] = value(m.fs.separator.outlet_to_diluate[0].conc_mass_phase_comp["Liq", ion])
#         conc_conc[ion] = value(m.fs.separator.outlet_to_concentrate[0].conc_mass_phase_comp["Liq", ion])

#     # Sequential initialization with looser then tighter tolerances
#     m.fs.EDstack.initialize(optarg={"tol": 1e-3, "max_iter": 50})
#     m.fs.EDstack.initialize(optarg={"tol": 1e-6})
...
def get_ion_config(ions):
    ...
    if not isinstance(ions, (list, tuple)):  # CONSIDERING ION LIST AS TUPLE
        ions = [ions]
    ...
    # ADDING ELEC_MOBILITY_DATA BASED ON ELECTRODIALYSIS_1STACK.PY
    elec_mobility_data = {
        ("Liq", "Na_+"): 5.19e-8,
        ("Liq", "Cl_-"): 7.92e-8,
        ("Liq", "NH4_+"): 7.6e-8,  # ADDED, value from Claude3.5, need ref
    }
    ...
    for ion in ions:
        ...
        if ("Liq", ion) in elec_mobility_data:
            ion_config["elec_mobility_data"][("Liq", ion)] = elec_mobility_data[
                ("Liq", ion)
            ]
    ...
    stream_table = create_stream_table_dataframe(
        {
            ...
            "IX Outlet": m.fs.ix.process_flow.outlet,
            "ED Feed": m.fs.ed.feed_inlet,
            "ED Diluate": m.fs.ed.diluate_outlet,
            "ED Concentrate": m.fs.ed.concentrate_outlet,
            "Product": m.fs.product.inlet,
            "Waste": m.fs.waste.inlet,
        },
        time_point=0,
    )
...
from ...tools.plot_network import plot_network

plot_network(m, stream_table, path_to_save=None)
